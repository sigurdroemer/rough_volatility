...
# Remark: The order is (H,nu,rho,xi0,xi1,xi2,...,xi28)
model$Txi

# Check non-negativity requirement:
t <- model$Txi[2:n]
theta <- GetThetaFromXi(v0,H,t,xi[2:n])
valid <- CheckNonNegReqTheta(v0,H,t,theta)[[1]]
valid
...
parCalib <- calib$par[idxConvPar]
parCalib
parTrue

# Check non-negativity requirement:
# - Ideally, this should be included in the optimisation problem.
theta <- GetThetaFromXi(parCalib[4],parCalib[1],t,parCalib[5:length(parCalib)])
valid <- CheckNonNegReqTheta(parCalib[4],parCalib[1],t,theta)[[1]]
valid
