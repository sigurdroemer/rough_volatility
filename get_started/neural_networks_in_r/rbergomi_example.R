# Description: Shows how to use the rough Bergomi neural network implementation.
...
weights_folder <- gsub("/","//",paste(project_folder,"/code/neural_networks/data/neural_network_weights/rbergomi",sep=""))
...
# Load model:
model <- LoadModel(contract_grid_folder,weights_folder,"rbergomi")
...
# Set parameters:
par <- c(0.1,2.1,-0.90,rep(0.10^2,length(model$Txi)))

# Remark: The order is (H,eta,rho,xi1,xi2,...,xi27) where the forward variance curve 
...
parTrue <- c(0.1,2.1,-0.90,rep(0.15^2,length(model$Txi)))
